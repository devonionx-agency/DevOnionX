  ...
  useEffect(() => {
    ...
    function buildWaterfall() {
      ...
      const tl = gsap.timeline({
        onComplete: () => {
          ...
          stateRef.current.loopTimer = gsap.delayedCall(
            cfg.loopDelay,
            buildWaterfall,
          );
        },
    ...
    const visibilityObserver = new IntersectionObserver(
      (entries) => {
        ...
        if (isVisible) {
          stateRef.current.gsapTl?.resume();
          stateRef.current.loopTimer?.resume();

          startRenderLoop();
        } else {
          stateRef.current.gsapTl?.pause();
          stateRef.current.loopTimer?.pause();
    ...
    return () => {
      ...
      stateRef.current.gsapTl = null;

      stateRef.current.loopTimer?.kill();
      stateRef.current.loopTimer = null;
    ...
    // Keep the original dependency behavior:
    // plates/badges control canvas + animation setup.
